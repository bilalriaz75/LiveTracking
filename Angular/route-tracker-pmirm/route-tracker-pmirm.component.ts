...
export class RouteTrackerPMIRMComponent implements OnInit {
  ...
  origin: any;
  destination: any;
  waypoints = [{ location: { lat: 24.9379389, lng: 67.1514844 } }, { location: { lat: 41.8339037, lng: -87.8720468 } }];
  speedMultiplier = 10;
  map: any;
  line: any;
  ...
  filterLocationPinsWithTimeGap(locations: any[]): any[] {
    const filteredLocations = [];

    for (let i = 0; i < locations.length - 1; i++) {
      const currentTime = new Date(locations[i].visit_datetime).getTime();
      const nextTime = new Date(locations[i + 1].visit_datetime).getTime();
      const timeDifference = (nextTime - currentTime) / (1000 * 60); // Convert to minutes

      if (timeDifference > 30) {
        filteredLocations.push(locations[i]);
        filteredLocations.push(locations[i + 1]);
      }
    }

    // Optionally, include the last location if needed

    return filteredLocations;
  }

  objectGenerator(type: String) {
  ...
  getShopsTrackingBySurveyorId(p) {
    ...
    console.log('this.objp: ', this.objp);
    this.httpService.getShopsForSurveyorLiveTrackingById(this.objp).subscribe((res: any) => {
      if (res.length <= 0) {
        ...
        this.planned = this.trackedShops2.filter((e) => e.Type == 'planned').length;
        console.log(
          'planned',
          this.trackedShops2.filter((e) => e.Type == 'planned')
        );
        this.locationTypePins = this.trackedShops2
        ...
        this.distancedShops = this.filterLocationPinsWithTimeGap(this.locationTypePins);
        console.log(this.distancedShops, 'latepins');
        this.unplanned = this.trackedShops2.filter((e) => e.Type == 'unplanned').length;
        console.log(
          'unplanned',
          this.trackedShops2.filter((e) => e.Type == 'unplanned')
        );
        this.visited = this.trackedShops2.filter((e) => e.Type == 'Visit').length;
        ...
        for (let i = 0; i < this.merchandiserLocations.length - 1; i++) {
          const { latitude: lat1, longitude: lon1 } = this.merchandiserLocations[i];
          const { latitude: lat2, longitude: lon2 } = this.merchandiserLocations[i + 1];
          this.totalDistance += this.calculateDistance(lat1, lon1, lat2, lon2);
        }

        console.log('Total distance covered:', this.totalDistance.toFixed(2), 'kilometers');
  ...
  public markerClicked = (markerObj) => {
    if (this.map) this.map.setCenter({ lat: markerObj.latitude, lng: markerObj.longitude });
  ...
  hasVisitedShop(shop: any): boolean {
    return this.trackedShops2.some(
      (visitedShop) =>
        visitedShop.remarks_id === 1 &&
        visitedShop.latitude === shop.latitude &&
        visitedShop.longitude === shop.longitude
    );
  }
