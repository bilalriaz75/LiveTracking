...
export class VoLiveTrackingComponent implements OnInit {
  ...
  goToEvaluation(shop) {
    ...
    if (this.projectType == 'Resource Management') {
      window.open(
        `${environment.hash}dashboard/virtual_view/list/route-tracker-pmirm?surveyorId=${
          shop.surveyorId
        }&startDate=${this.modifyDate(shop.visit_date)}&userType=${localStorage.getItem('user_id')}`,
        '_blank'
      );
    } else {
      window.open(
        `${environment.hash}dashboard/virtual_view/list/route-tracker?surveyorId=${
          shop.surveyorId
        }&startDate=${this.modifyDate(shop.visit_date)}&userType=${localStorage.getItem('user_id')}`,
        '_blank'
      );
  ...
  selectedUpdateChange() {
    ...
    if (this.selectedUpdate.id == 1) {
      this.trackedShops2 = this.trackedShops.filter((t) => t.last_update_before <= 30);
    } else if (this.selectedUpdate.id == 2) {
      this.trackedShops2 = this.trackedShops.filter((t) => t.last_update_before > 30 && t.last_update_before <= 120);
    } else if (this.selectedUpdate.id == 3) {
      this.trackedShops2 = this.trackedShops.filter((t) => t.last_update_before > 120);
    } else {
